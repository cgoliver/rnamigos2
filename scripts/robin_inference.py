...
def get_nodelists_and_ligands(robin_systems=ROBIN_SYSTEMS, pockets_path='data/json_pockets_expanded'):
    ...
    for robin_sys in robin_systems.splitlines():
        ...
        if len(copies) == 0:
            ...
            ligand_names[representative_pocket] = f"{ligand_name}_decoys.txt"
...
if __name__ == "__main__":
    ...
    for pocket, ligands in ligand_names.items():
        pocket_name = pocket.strip('.json')
        print('Doing pocket : ', pocket)
        ...
        ligands_path = os.path.join(decoys_ligands_dir, ligands)
        ...
        inference(dgl_graph=dgl_pocket_graph, smiles_list=smiles_list, out_path=out_path,
                  dump_all=True, model_path=models_path)

        active_ligands_path = os.path.join("data", "ligand_db", pocket_name, "robin", "active.txt")
        ...
        inactives_ligands_path = os.path.join("data", "ligand_db", pocket_name, "robin", "inactive.txt")
