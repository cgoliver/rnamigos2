...
def validate_affinity_classification_model(run, graph_dir, folds=1):
    ...
    rdock_scores = pd.DataFrame(columns=['POCKET_ID', 'TRUE_CLASS', 'PREDICTED_CLASS','ELAPSED_TIME','ELAPSED_TIME_2'])
    true_label = []
    predicted_label = []
    #graph_dir = 'data/annotated/pockets_docking_annotated_cons_val'
    graph_dir = 'data/annotated/pockets_docking_annotated_inter_cleaned_nat_bal_valset'
    ...
    #graph_dir = 'data/annotated/pockets_docking_annotated_inter_cleaned_nat_bal'
    fp_dict = {}
    pred_dict = {}
    res_model = []
    res_model_dict = {}
    for fold in range(int(folds)):
        ...
        test_graphlist = pickle.load(open('data/annotated/consolidate_dataset_validation_native_ligand_balanced.p', 'rb'))
        test_graph_path=graph_dir
        nm = 0            
        for g_path in test_graphlist:
            nm += 1
            ...
            with torch.no_grad():
                ...
                rdock_scores = pd.concat([rdock_scores, pd.DataFrame([{'POCKET_ID': g_path,
                    'TRUE_CLASS': str(is_native),
                    'PREDICTED_CLASS': str(fp_pred[0][0]),
                    'ELAPSED_TIME':str(t1),
    ...
    #rdock_scores.to_csv('predictions_ABPN_affinity_class_model_consolidate_natbal_1_hung_testset.csv')
    rdock_scores.to_csv('pred_ABPN_affinity_class_model_consolidate_natbal_1_valset.csv')
