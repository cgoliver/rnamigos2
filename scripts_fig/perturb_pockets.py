...
def get_perturbed_pockets(
    ...
    for pocket in tqdm(pockets_to_compute):
        ...
        around_pocket = bfs(
            rglib_graph, in_pocket_filtered, depth=perturb_bfs_depth, label="LW"
        )
...
def compute_efs_model(model, dataloader, lower_is_better):
    ...
    for ef, score, pocket_id in zip(efs, scores, pocket_names):
        grouped = {"score": ef, "pocket_id": pocket_id}
        rows.append(grouped)
...
def get_perf(pocket_path, base_name=None, out_dir=None):
    ...
    native_model_path = (
        "results/trained_models/is_native/native_pretrain_new_pdbchembl_rnafm"
    )
    native_model = get_model_from_dirpath(native_model_path)

    # Get dock performance
    df_dock, df_dock_raw, df_dock_ef = compute_efs_model(
        dock_model, dataloader=dataloader, lower_is_better=True
    )
    ...
    df_native, df_native_raw, df_native_ef = compute_efs_model(
    ...
    big_df_raw = df_dock_raw.merge(
        df_native_raw, on=["pocket_id", "smiles", "is_active"], how="outer"
    )
    ...
    mixed_df, mixed_df_raw, mixed_df_ef = mix_two_scores(
        big_df_raw, score1="dock", score2="native"
    )
...
def get_results_dfs(
    ...
    todo = list(
        sorted(
            [
                x
                for x in os.listdir(all_perturbed_pockets_path)
...
def get_all_perturbed_soft(
    ...
    out_df = (
        f'figs/perturbations/aggregated_soft{"_robin" if ROBIN else ""}_{final_bfs}.csv'
    )
    if not use_cached_pockets:
        get_perturbed_pockets(
            out_path=out_path,
...
def main_chembl():
    ...
    dfs_random = get_all_perturbed_bfs(
        fractions=fractions,
        recompute=recompute,
    ...
    dfs_hard = get_all_perturbed_bfs(
        fractions=fractions,
        recompute=recompute,
        use_cached_pockets=use_cached_pockets,
        hard=True,
    ...
    df_soft_1 = get_all_perturbed_soft(
        fractions=fractions,
        use_cached_pockets=use_cached_pockets,
        final_bfs=1,
        recompute=recompute,
    ...
    df_soft_2 = get_all_perturbed_soft(
        fractions=fractions,
        use_cached_pockets=use_cached_pockets,
        final_bfs=2,
        recompute=recompute,
    ...
    df_soft_3 = get_all_perturbed_soft(
        fractions=fractions,
        use_cached_pockets=use_cached_pockets,
        final_bfs=3,
        recompute=recompute,
    ...
    df_soft_4 = get_all_perturbed_soft(
        fractions=fractions,
        use_cached_pockets=use_cached_pockets,
        final_bfs=4,
        recompute=recompute,
    ...
    dfs_soft = [df_soft_1, df_soft_2, df_soft_3, df_soft_4]
...
def main_robin():
    ...
    DF_UNPERTURBED = pd.read_csv(
        "figs/perturbations/unperturbed_robin/json_pockets_expanded_mixed.csv",
        index_col=False,
    )
