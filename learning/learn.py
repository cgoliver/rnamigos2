...
def test(model, test_loader, device, fp_lam=1, rec_lam=1):
    ...
    for batch_idx, (graph, K, fp, inter_score, sample_type, idx) in enumerate(test_loader):
        ...
        fp = fp.to(device)
        inter_score = inter_score.to(device)
        ...
        with torch.no_grad():
            ...
            loss = model.compute_loss(inter_score, fp_pred, embeddings, K,
                                        fp_lam=fp_lam,
                                        rec_lam=rec_lam)
        del K
        del graph
...
def train_model(model, criterion, optimizer, device, train_loader, test_loader, save_path,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (graph, K, fp, inter_score, sample_type, idx) in enumerate(train_loader):
