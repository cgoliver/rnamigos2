...
def test(model, test_loader, device, fp_lam=1, rec_lam=1):
    ...
    for batch_idx, (graph, K, fp, label_native_lig, sample_type, idx) in enumerate(test_loader):
        ...
        fp = fp.to(device)
        label_native_lig = label_native_lig.to(device)
        ...
        with torch.no_grad():
            fp_pred, embeddings = model(graph, fp)
            loss = model.compute_loss(label_native_lig, fp_pred, embeddings, K,
                                        fp_lam=fp_lam,
                                        rec_lam=rec_lam)
        del K
        del graph
...
def train_model(model, criterion, optimizer, device, train_loader, test_loader, save_path,
    ...
    learning_curve_val_df = pd.DataFrame(columns=['EPOCH', 'LOSS','TYPE_LOSS'])
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (graph, K, fp, label_native_lig, sample_type, idx) in enumerate(train_loader):
    ...
    print(learning_curve_val_df)
    pickle.dump(learning_curve_val_df, open('learning_curve_rnamigos_raw.p', 'wb'))
