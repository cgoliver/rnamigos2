...
class Model(nn.Module):
    def __init__(self, dims, device, attributor_dims, num_rels, pool='att', num_bases=-1,
        ...
        if pool == 'att':
            ...
            pooling_gate_nn = nn.Linear(self.dims[0]+1, 1)
        ...
        #print('******* model constructor attributor_dims*******')
        #print(attributor_dims)
        #print('***** dims ******')
        #print(self.dims)
        #self.dims[2] += 166
        
        #attributor_dims[0] = attributor_dims[0] + 166
    ...
    def forward(self, g, lig_fp):
        ...
        if self.nucs:
            ...
            #embeddings = torch.cat((embeddings, lig_fp.float()), 1)

        #print('-------embeddings.shape------')
        #print(embeddings.shape)
        #print('**** lig_fp before reshape ****** ')
        #print(lig_fp.shape)
        #embeddings = torch.cat((embeddings[1], lig_fp.float()), 0)

        fp = self.pool(g, embeddings)
        #print('****** after pooling fp shape ********')
        #print(fp.shape)
        #print('**** lig_fp before reshape ****** ')
        #print(lig_fp.shape)
        fp = torch.cat((fp, lig_fp.float()), 1)
        #print('*** fingerprint after concat *******')
        #print(fp.shape)
    ...
    def fp_loss(self, target_fp, pred_fp):
        ...
        #print('loss function ------')
        #print(pred_fp.size())
        #print(target_fp.size())
        #print(pred_fp[7][0].item())
        #print(target_fp[7][0].item())
        #loss = torch.nn.BCELoss()(pred_fp, target_fp)
        #loss = torch.nn.MSELoss()(pred_fp, target_fp)
        loss = torch.nn.L1Loss()(pred_fp, target_fp)
        return loss
    def compute_loss(self, target_fp, pred_fp, embeddings, target_K, 
        ...
        #loss = fp_lam * self.fp_loss(target_fp.reshape(-1, 1), pred_fp)
        #print(target_fp.size())
        #print(y.size())
        target_fp = torch.unsqueeze(target_fp, 1)
        #print('in loss******** ')
        #print(pred_fp)
        loss = fp_lam * self.fp_loss(target_fp, pred_fp)
        return loss
