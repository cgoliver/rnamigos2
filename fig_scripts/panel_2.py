...
def train_sim_perf_plot(rm_scores, grouped=True):
    """
    ...
    mixed_res['train_sim_max'] = mixed_res['pocket_id'].apply(
        lambda x: rm_scores.loc[x, rm_scores.index.isin(names_train)].max())
...
def compute_pred_distances(big_df_raw, out_name="outputs/pred_mixed_overlap_vincent.csv"):
    """
    Compute the pariwise distance between all pockets
    The similarity between two pockets is based on our predictions: it is computed as the IoU of the top 5%
    """
    test_pockets = sorted(list(set(big_df_raw['pocket_id'])))
    ref_ligs = list(set(big_df_raw.loc[big_df_raw['pocket_id'] == '1BYJ_A_GE3_30']['smiles']))
    pocket_pairs = list(itertools.combinations(test_pockets, 2))

    def get_score_pocket(pocket, scores, ref_ligs, percentile=.05):
    ...
    for pocket in test_pockets:
        pocket_preds[pocket] = get_score_pocket(pocket, ref_ligs=ref_ligs, scores=big_df_raw)
...
def double_heatmap(corr1, corr2=None,
    ...
    if corr2 is None:
        corr2 = corr1.T
        default_kwargs_2 = default_kwargs_1
...
def sims(grouped=True):
    ...
    # PLOT 1
    train_sim_perf_plot(rm_scores=rm_scores, grouped=grouped)

    # HEATMAPS

    # COMPUTE PDIST based on overlap of predictions
    out_name = "outputs/pred_mixed_overlap_vincent.csv"
    # compute_pred_distances(big_df_raw=big_df_raw, out_name=out_name)
    results_df = pd.read_csv(out_name)
    corrs = list(results_df['overlap'])
    square_corrs = np.zeros((len(test_pockets), len(test_pockets)))
    square_corrs[np.triu_indices(len(test_pockets), 1)] = corrs
    ...
    square_rms = np.zeros((len(test_pockets), len(test_pockets)))
    square_rms[np.triu_indices(len(test_pockets), 1)] = rms
    ...
    # ax = double_heatmap(corr1=square_corrs, corr2=square_rms.T, kwargs2={'vmax': 0.7, 'vmin': 0.2})
    # plt.show()
    ...
    square_tani = np.zeros((len(test_pockets), len(test_pockets)))
    square_tani[np.triu_indices(len(test_pockets), 1)] = tani
    palette_lig = sns.light_palette('orange', as_cmap=True)

    # ax = double_heatmap(corr1=square_corrs, corr2=square_tani.T, kwargs2={'cmap': palette_lig})
    # plt.show()
